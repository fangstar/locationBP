...
public class PlacesUpdateService extends IntentService {
	...
	protected void refreshPlaces(Location location, int radius) {
		...
		try {
			...
			String placesFeed = baseURI + "&location=" + locationStr + "&radius="
					+ radius + PlacesConstants.PLACES_API_KEY;
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				while (eventType != XmlPullParser.END_DOCUMENT) {
					if (eventType == XmlPullParser.START_TAG
						...
						String icon = "";
						String reference = "";
						while (!(eventType == XmlPullParser.END_TAG && xpp.getName()
								.equals("result"))) {
							if (eventType == XmlPullParser.START_TAG
						...
						addPlace(location, id, name, vicinity, types, placeLocation,
								viewport, icon, reference, currentTime);
					}
					eventType = xpp.next();
				}

				// Remove places from the PlacesContentProviderlist that aren't from
				// this updte.
				// String where = PlaceDetailsContentProvider.KEY_LAST_UPDATE_TIME +
				// " < " + currentTime;
				// contentResolver.delete(PlacesContentProvider.CONTENT_URI, where,
				// null);

				// Save the last update time and place to the Shared Preferences.
				prefsEditor.putLong(PlacesConstants.SP_KEY_LAST_LIST_UPDATE_LAT,
						(long) location.getLatitude());
				prefsEditor.putLong(PlacesConstants.SP_KEY_LAST_LIST_UPDATE_LNG,
						(long) location.getLongitude());
				prefsEditor.putLong(PlacesConstants.SP_KEY_LAST_LIST_UPDATE_TIME,
	...
	/**
	 * Adds the new place to the {@link PlacesContentProvider} using the values
	...
	protected boolean addPlace(Location currentLocation, String id, String name,
			String vicinity, String types, Location location, String viewport,
			String icon, String reference, long currentTime) {
		/*
		 * // Contruct the Content Values ContentValues values = new
		 * ContentValues(); values.put(PlacesContentProvider.KEY_ID, id);
		 * values.put(PlacesContentProvider.KEY_NAME, name); double lat =
		 * location.getLatitude(); double lng = location.getLongitude();
		 ...
		 * location float distance = 0; if (currentLocation != null && location !=
		 * null) distance = currentLocation.distanceTo(location);
		 * values.put(PlacesContentProvider.KEY_DISTANCE, distance);
		 * 
		 * // Update or add the new place to the PlacesContentProvider String where
		 * = PlacesContentProvider.KEY_ID + " = '" + id + "'"; boolean result =
		 * false; try { if
		 * (contentResolver.update(PlacesContentProvider.CONTENT_URI, values, where,
		 * null) == 0) { if
		 * (contentResolver.insert(PlacesContentProvider.CONTENT_URI, values) !=
		 * null) result = true; } else result = true; } catch (Exception ex) {
		...
		if ((prefetchCount < PlacesConstants.PREFETCH_LIMIT)
			 ...
			 * prefetchCount++;
			 */
			// Start the PlaceDetailsUpdateService to prefetch the details for this
			// place.
			 ...
			 * reference); updateServiceIntent.putExtra(PlacesConstants.EXTRA_KEY_ID,
			 * id);
			 * updateServiceIntent.putExtra(PlacesConstants.EXTRA_KEY_FORCEREFRESH,
			 * false); startService(updateServiceIntent);
			 */
		}
	...
	/**
	 * Remove stale place detail records unless we've set the persistent cache
	 * flag to true. This is typically the case where a place has actually been
	 * viewed rather than prefetched.
	 * 
	 * @param location
	 *          Location
	 * @param radius
	...
	protected void removeOldLocations(Location location, int radius) {
		...
		long minTime = System.currentTimeMillis()
				- PlacesConstants.MAX_DETAILS_UPDATE_LATENCY;
		/*
		 * String where = PlaceDetailsContentProvider.KEY_LAST_UPDATE_TIME + " < " +
		 * minTime + " AND " + PlaceDetailsContentProvider.KEY_FORCE_CACHE + " = 0";
		 * contentResolver.delete(PlaceDetailsContentProvider.CONTENT_URI, where,
		 * null);
		 */
	}
}